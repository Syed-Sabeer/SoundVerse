              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header card-no-border text-end">
                    <div class="card-header-right-icon"><a class="btn btn-primary f-w-500" href="{{route('admin.ads.create')}}"><i class="fa fa-plus pe-2"></i>Add Ad</a></div>
                  </div>
                        ...
                        <table class="table" >
                          <thead>
                            <tr>
                              <th></th>
                              <th>No.</th>
                              <th> <span class="c-o-light f-w-600">File</span></th>
                              <th> <span class="c-o-light f-w-600">Title</span></th>
                              <th> <span class="c-o-light f-w-600">Link</span></th>
                            ...
                            <tr class="product-removes">
                              ...
                              <td>
                                @if($ad->file)
                                  ...
                                  @endphp
                                  @if($isVideo)
                                    <video width="50" height="50" controls style="border-radius: 5px;">
                                      <source src="{{ $ad->file_url }}" type="video/{{ $extension }}">
                                    </video>
                                  @else
                                    <img src="{{ $ad->file_url }}" alt="{{ $ad->title }}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 5px;">
                                  @endif
                                @else
                                ...
                                <td>
                                <p class="c-o-light">{{$ad->title}}</p>
                              </td>
                              <td>
                                <p class="c-o-light">{{ \Illuminate\Support\Str::limit($ad->link, 30, '...') }}</p>
                              </td>
                           
                              <td>
<form method="POST" action="{{ route('admin.ads.toggleVisibility', $ad->id) }}" style="display:inline;" class="toggle-visibility-form">
    @csrf
    <input type="hidden" name="visibility" value="0">
    <div class="form-check form-switch form-check-inline">
        <input class="form-check-input switch-primary check-size" type="checkbox" role="switch" name="visibility" value="1" {{ $ad->visibility ? 'checked' : '' }} onchange="this.form.submit()">
    </div>
</form>
  ...
  <a class="square-white" href="{{ route('admin.ads.edit', $ad->id) }}">
    <svg>
