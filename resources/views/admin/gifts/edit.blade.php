      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h5>Edit Gift Form</h5>
          </div>
          <div class="card-body">
            <form class="row g-3 common-form" method="POST" action="{{ route('admin.gift.update', $gift->id) }}">
              @csrf
              @method('PUT')

              <div class="col-md-12">
                <label class="form-label">Gift Icon</label>
                <input class="form-control" name="icon" type="text" value="{{ old('icon', $gift->icon) }}">
              </div>

              <div class="col-md-12">
                <label class="form-label">Gift Title</label>
                <input class="form-control" name="title" type="text" value="{{ old('title', $gift->title) }}">
              </div>

              <div class="col-md-12">
                <label class="form-label">Gift Rings Count</label>
                <input class="form-control" name="rings_count" type="text" value="{{ old('rings_count', $gift->rings_count) }}">
              </div>

                      <div class="col-md-12">
                <label class="form-label">Gift Couples Connected</label>
                <input class="form-control" name="couples_connected" type="text" value="{{ old('couples_connected', $gift->couples_connected) }}">
              </div>

              <div class="col-md-12">
                <label class="form-label">Price</label>
                <input class="form-control" name="price" type="text" value="{{ old('price', $gift->price) }}">
              </div>

              <div class="col-md-12">
                <label class="form-label">Gift Description</label>
                <textarea id="gift_description" name="description" class="form-control" rows="6">{{ old('description', $gift->description) }}</textarea>
              </div>
              ...
              <div class="col-md-6 mt-3">
                <button class="btn btn-primary f-w-500" type="submit">Update Gift</button>
              </div>
            </form>
          </div>
        </div>
