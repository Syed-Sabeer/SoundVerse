...
<div class="signup-app mt-5">
    ...
    <div class="container">
        ...
        <div class="right-section">
            ...
            <form id="resetPasswordForm" method="POST" action="{{ route('password.reset.post') }}">
                @csrf
                <input type="hidden" name="email" value="{{ session('reset_email') ?? old('email') }}">
                <input type="hidden" name="otp" value="{{ session('verified_otp') ?? old('otp') }}">
                <input type="hidden" name="is_artist" id="is_artist" value="{{ session('reset_is_artist') ?? request('is_artist', 0) }}">
                
                <div class="form-group">
                    <input type="password" name="password" id="password" class="form-input" placeholder="New Password" required minlength="8">
                </div>

                <div class="form-group">
                    <input type="password" name="password_confirmation" id="password_confirmation" class="form-input" placeholder="Confirm New Password" required minlength="8">
                </div>
